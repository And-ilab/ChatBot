...
const loadMessages = async () => {
  try {
    chatMessages.style.height = "550px";
    const messagesResponse = await apiFetch(`/api/messages/${dialogID}/`);
...
const loadArtifact = async (artifactType, artifactID) => {
  ...
  try {
    const response = await apiFetch(
      `/api/get-artifact-by-id/?artifactID=${encodedArtifactID}&artifactType=${encodedArtifactType}`,
      { method: "GET" }
    );
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Error fetching answer: ${response.status} - ${errorText}`
      );
    }
...
const appendMessage = (sender, content, timestamp, showButton = false) => {
  ...
  messageDiv.className =
    sender === "bot" ? "message bot-message" : "message user-message";

  const date = new Date(timestamp);
  date.setHours(date.getHours() + 3);
  const time = date.toTimeString().slice(0, 5);
  const timeClass =
    sender === "bot" ? "message-time" : "message-time user-message-time";
  ...
  if (
    messages.length === 0 ||
    messageDate !== messages[messages.length - 1].dataset.date
  ) {
...
const sendMessageToAPI = async (
  ...
  await apiFetch(`/api/send-message/${dialog_id}/`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      "X-CSRFToken": csrfToken,
...
const fetchNodes = async (type) => {
  ...
  try {
    const response = await apiFetch(
      `/api/get-nodes-by-type/?type=${encodedType}`,
      { method: "GET" }
    );
    ...
    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(
        `Error fetching nodes: ${response.status} - ${errorText}`
      );
    }
    ...
    if (data && Array.isArray(data.result)) {
      ...
      throw new Error(
        'Unexpected response format: missing or malformed "result"'
      );
    }
  } catch (error) {
...
const fetchNodesWithRelation = async (
  startNodeType,
  startNodeName,
  finishNodeType
) => {
  ...
  try {
    const response = await apiFetch(
      `/api/get-nodes-by-type-with-relation/?startNodeType=${encodeStartNodeType}&startNodeName=${encodeStartNodeName}&finishNodeType=${encodeFinishNodeType}`,
      { method: "GET" }
    );
...
const fetchAnswer = async (questionId) => {
  ...
  try {
    const response = await apiFetch(
      `/api/get-answer/?questionId=${encodedQuestionId}`,
      { method: "GET" }
    );
...
const fetchDocuments = async (answerID) => {
  ...
  try {
    const response = await apiFetch(
      `/api/get-documents/?answerID=${encodedAnswerID}`,
      { method: "GET" }
    );
    if (!response.ok) {
      const errorText = await response.text();
...
const createDocumentBlock = async (documents) => {
  ...
  documents.forEach(async (doc) => {
    ...
    if (doc.type === "link") {
      ...
      try {
        const response = await apiFetch(
          `/api/get-document-link-by-name/${doc.name}/`
        );
        if (!response.ok) {
        ...
        console.error(
          `Ошибка при получении ссылки для файла "${doc.name}":`,
          error.message
        );
...
const userResponseHandler = async (message) => {
  ...
  const isGreeting = greetings.some((greeting) =>
    cleanedMessage.includes(greeting)
  );
  ...
  try {
    ...
    const recognizeResponse = await apiFetch("/api/recognize-question/", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRFToken": csrfToken,
    ...
    if (!recognizeResponse.ok) {
      throw new Error(
        `Ошибка: ${recognizeResponse.status} ${recognizeResponse.statusText}`
      );
    }
    ...
    if (data.recognized_question) {
      ...
      const encodedQuestionContent = encodeURIComponent(
        data.recognized_question
      );
      try {
        const questionIDResponse = await apiFetch(
          `/api/get-question-id-by-content/?questionContent=${encodedQuestionContent}`,
          { method: "GET" }
        );
      ...
      const operatorButton =
        operatorButtonContainer.querySelector(".operator-button");
      operatorButton.addEventListener(
        "click",
        async (e) => {
          const confirmMessage =
            operatorConfirmationMessages[
...
const showTopicButtons = async (sectionName) => {
  ...
  createButtonsFromNodes(nodes, async (selectedNode) => {
    ...
    navigationStack.push({
      type: "Topic",
      name: selectedNode.name,
      fetchFunction: fetchNodesWithRelation,
    });
...
const showAnswer = async (questionID) => {
  ...
  if (answer) {
    ...
    let message =
      "Не удалось найти ответ на выбранный вопрос, попробуйте еще раз";
...
async function getLatestDialog(userId) {
  try {
    const response = await apiFetch(`/api/dialogs/latest/${userId}/`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
      },
...
async function createNewDialog(userId) {
  try {
    const response = await apiFetch(`/api/dialogs/create/${userId}/`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "X-CSRFToken": csrfToken,
